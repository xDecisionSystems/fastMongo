...
MONGO_WRITER_USERNAME="${MONGO_WRITER_USERNAME:-writer}"
MONGO_READER_USERNAME="${MONGO_READER_USERNAME:-reader}"
...
WRITE_API_KEY="${WRITE_API_KEY:-$(openssl rand -hex 32)}"
EXPORT_API_KEY="${EXPORT_API_KEY:-$(openssl rand -hex 32)}"
GENERATE_API_KEY="${GENERATE_API_KEY:-$(openssl rand -hex 32)}"
CORS_ORIGINS="${CORS_ORIGINS:-}"
GETRECS_ALLOWED_FIELDS="${GETRECS_ALLOWED_FIELDS:-package.name,package.version}"
...
# - MONGO_WRITER_USERNAME / MONGO_WRITER_PASSWORD:
#   Must match an existing MongoDB user, or update the Mongo user credentials too.
# - MONGO_DB_NAME:
...
MONGO_WRITER_USERNAME=${MONGO_WRITER_USERNAME}
